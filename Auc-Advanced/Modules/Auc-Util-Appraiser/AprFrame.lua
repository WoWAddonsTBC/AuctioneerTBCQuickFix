...
function private.CreateFrames()
	...
	function frame.ChangeUI()
		if get("util.appraiser.classic") then
			...
			frame.salebox.bid.stack:Show()
			frame.salebox.buy.stack:Show()
			if frame.salebox.SetBackdropColor then
...
--		frame.DirectSelect(link)
	...
	if frame.itembox.SetBackdrop then
	...
	if frame.salebox.SetBackdrop then
   ...
   -- Would rather the distance here matched the length of the "All Stacks" text and was recalculated.
	...
	frame.salebox.bid:Hide()
	if AppraiserSaleboxBidGold.SetBackdrop then
	...
	frame.salebox.bid.stack:Hide()
	if AppraiserSaleboxBidStackGold.SetBackdrop then
	...
	frame.salebox.buy:Hide()
	if AppraiserSaleboxBuyGold.SetBackdrop then
	...
	frame.salebox.buy.stack:Hide()
	if AppraiserSaleboxBuyStackGold.SetBackdrop then
	...
	if frame.manifest.SetBackdrop then
	...
	if frame.imageview.SetBackdrop then
	...
	if frame.imageview.purchase.SetBackdrop then
